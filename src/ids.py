...
# local imports not required for generic functions

# reuse Node and stats structures similar to bfs
...
def depth_limited_search(problem, limit: Optional[int] = 5, return_stats: bool = False):
    """Stack-based depth-limited search.

    Returns path (list of (state, action)) or (path, stats) if return_stats True.
    """
    stats = IDSStats()
    # stack stores Node objects directly; no global explored list
    start = problem.start
    ...
    while stack:
        node = stack.pop()
        depth = node.depth
        # do not expand beyond limit; still test goal
        ...
        # push children onto stack (reverse order so first action is explored first)
        children: List[_Node] = []
        ...
        # push in reverse so Actions order is preserved
...
def ids(problem, max_limit: Optional[int] = 50, return_stats: bool = False):
    """Iterative deepening search using depth_limited_search repeatedly up to max_limit.

    When return_stats=True, accumulates IDSStats across iterations until a solution is found.
    """
    ...
    for depth in range(0, max_limit + 1):
        ...
        # if found, set solution info and return
        if path:
